...
The 5-Model ATS Evaluation: Agent 4 runs the draft through 5 independent simulated parsers. If Score < 90, sends a diff-patch back to Agent 2.

Visual Validation (The Eye): Agent 6 renders the output to simulate the PDF conversion. It checks for "orphaned" headers at the bottom of pages or list breaks. If issues are found, it sends a CSS/HTML layout fix back to Agent 3.

Resume Output & Score Display: The system outputs the final, print-ready Markdown/HTML resume (strictly no top-level navigation/TOC; includes CSS pagination hygiene and HTML-hyperlinked contact info) along with a brief breakdown: "Final ATS Score: 94/100 (Evaluated across 5 models)."
